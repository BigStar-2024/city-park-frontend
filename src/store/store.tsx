...
import {
  ...
  import { useNavigate } from "react-router-dom";
  import axios from "axios";
  export type userPrimitiveType = User & {
    customClaims: {
      admin?: boolean;
    };
  };
  ...
  const StoreProvider = ({ children }: { children: JSX.Element }) => {
    ...
    const handleAuthorize = async (user: User | null) => {
      if (user && user.emailVerified) {
        ...
        user
          ...
          .then((idTokenResult) => {
            // Custom claims are accessible in idTokenResult.claims
            const { admin }: { admin: boolean } = idTokenResult.claims as {
              admin: boolean;
            };
            setUser((v) => (v ? { ...v, customClaims: { admin } } : null));
          })
    ...
    return (
      <GlobalContext.Provider
        value={{ authorize, logout, sideBarOpen, setSideBarOpen, user }}
      >
        {content}
      </GlobalContext.Provider>
    );
