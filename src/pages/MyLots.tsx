...
const Lot = ({
  ...
  const handleClick = async () => {
    if (editable) {
      const res = await axios.put(`/lot/${lot._id}`, {
        owners: lotOwners.map((owner) => owner.email),
      });
      if (res.status === 200) showToast("Updated owners successfully", true);
      await fetchLot();
    }
  ...
  return (
    <>
      <div
        ...
        className="flex flex-col justify-center items-center cursor-pointer group border border-black overflow-hidden"
      >
        <img
          className="group-hover:scale-[1.05] transition-all ease-in-out duration-1000"
          src={`${import.meta.env.VITE_API_BACKEND_URL}public/${lot.cover}`}
        />
          ...
          header={() => (
            <div className="flex items-center gap-4">
              <img
                ...
                src={`${import.meta.env.VITE_API_BACKEND_URL}public/${
                  lot.cover
                }`}
              />
              <span>{lot.siteCode}</span>
              {editable ? (
                <MultiSelect
                  ...
                  className="w-[700px] md:w-20rem text-xs"
                  display="chip"
                />
              ) : (
          ...
          <div className="flex py-2 items-center w-full">
            ...
            <div className="relative w-full h-fit flex items-center border border-black rounded-md bg-black/5">
              <button
                ...
                className="p-3 z-10 bg-[#f0f0f0] rounded-l-md border-r border-black hover:opacity-80 active:bg-orange-100 ease-in-out transition-all"
              >
                <svg width={20} height={20}>
                  <use href="#svg-copy" />
                </svg>
              </button>
              <div className="absolute left-0 w-full overflow-x-auto p-2 pl-[60px] text-xs text-nowrap">{`url=${
          ...
          <div>
            <div className="flex justify-between max-lg:flex-col gap-2 items-center pb-4 ">
              <div className="flex w-full items-center gap-1">
                ...
                <i className="w-full border border-black p-2 rounded-md min-w-[200px] overflow-x-auto block">
                  {lot.url}
                </i>
              </div>
            </div>
  ...
  return (
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-6">
        ...
        {lots.map((lot, i) => (
          <Lot key={i} lot={lot} fetchLot={fetchLot} endUsers={endUsers} />
        ))}
      </div>
      <Dialog
        pt={{
